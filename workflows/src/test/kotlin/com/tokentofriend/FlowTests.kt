...
class FlowTests {
    ...
    fun `Check if non fungible token correctly created`(){
        ...
        val storageNode = listOf(storedNonFungibleTokenb,storedNonFungibleTokenc,storedNonFungibleTokend,storedNonFungibleTokene)
                .filter { it.isNotEmpty() }.single().single().state.data
        println("-"+ storageNode.linearId+"-")
        assertEquals(storageNode.linearId, UniqueIdentifier(id = UUID.fromString(nonfungibleTokenId)))
